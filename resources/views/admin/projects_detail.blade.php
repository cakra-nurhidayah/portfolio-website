{{-- resources/views/admin/projects_detail.blade.php --}}
...
<div class="bg-white/80 rounded-2xl shadow p-6">
  <div class="flex items-center justify-between mb-6">
    <h2 class="text-xl md:text-2xl font-bold text-[#262525]">Detail Project</h2>
  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-xl p-6 shadow">
      <h3 class="text-2xl font-bold text-gray-800 mb-4">{{ $project->judul }}</h3>
      <p class="text-gray-600 mb-6">{{ $project->deskripsi }}</p>
      
      @if($project->gambar->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          @foreach($project->gambar as $gambar)
            <div class="relative group">
              <img src="{{ asset('storage/' . $gambar->url) }}" alt="{{ $gambar->caption }}" class="w-full h-48 object-cover rounded-lg">
              @if($gambar->caption)
                <div class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-75 text-white p-2 rounded-b-lg">
                  <p class="text-sm">{{ $gambar->caption }}</p>
                </div>
              @endif
            </div>
          @endforeach
    ...
    <!-- Rich Text Editor for Project Details -->
    <div class="bg-white rounded-xl p-6 shadow">
      <h3 class="text-xl font-bold text-gray-800 mb-4">Deskripsi Project</h3>
      ...
      <div class="border border-gray-300 rounded-lg mb-4 p-2 bg-gray-50">
        <div class="flex flex-wrap gap-2">
          ...
          <!-- Link -->
          <button onclick="insertLink()" class="px-3 py-12 bg-gray-200 hover:bg-gray-300 rounded text-sm">🔗</button>
      ...
      <div id="editor" contenteditable="true" class="min-h-96 p-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#F9B654] focus:border-transparent" 
        ...
        <p>Mulai menulis detail project Anda di sini...</p>
      </div>
      
      <!-- Inline Edit Form (merged from edit page, without deskripsi textarea) -->
      <form id="projectEditForm" action="{{ route('admin.projects.update', $project->id_projek) }}" method="POST" enctype="multipart/form-data" class="mt-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
          ...
          <input type="text" id="judul" name="judul" value="{{ old('judul', $project->judul) }}" 
        ...
        <div>
          ...
          <p class="mt-1 text-sm text-gray-500">Pilih gambar tambahan untuk project ini</p>
          @error('images.*')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
          @enderror
        </div>
...
function saveContent() {
  ...
  localStorage.setItem('project_detail_{{ $project->id_projek }}', content);
...
// List Functions - Fixed implementation
function insertList(type) {
  ...
  if (selectedText) {
    ...
    // If no text selected, create a new list item at cursor position
    const listItem = document.createElement('li');
    listItem.innerHTML = '&nbsp;'; // Non-breaking space to make it visible
