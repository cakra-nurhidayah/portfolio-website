{{-- resources/views/admin/services_edit.blade.php --}}
...
<div class="bg-white/80 rounded-2xl shadow p-6">
  <div class="flex items-center justify-between mb-6">
    <h2 class="text-xl md:text-2xl font-bold text-[#262525]">Edit Service</h2>
    <div class="flex gap-2">
      <a href="{{ route('admin.services') }}" class="px-5 py-2 rounded-xl bg-gray-500 text-white font-bold shadow hover:scale-105 transition">Kembali</a>
    </div>
  </div>
  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-xl p-6 shadow">
      <h3 class="text-2xl font-bold text-gray-800 mb-4">{{ $service->judul }}</h3>
      
      @if($service->url)
        <div class="relative group mb-4">
          <img src="{{ $service->url }}" alt="{{ $service->judul }}" class="w-full h-48 object-cover rounded-lg">
          @if($service->caption)
            <div class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-75 text-white p-2 rounded-b-lg">
              <p class="text-sm">{{ $service->caption }}</p>
            </div>
          @endif
        </div>
      @endif
    ...
    <div class="bg-white rounded-xl p-6 shadow">
      <h3 class="text-xl font-bold text-gray-800 mb-4">Deskripsi Service</h3>
      ...
      <div id="editor" contenteditable="true" class="min-h-96 p-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#F9B654] focus:border-transparent" 
        ...
        {!! old('deskripsi', $service->deskripsi) ?: '<p>Mulai menulis deskripsi service Anda di sini...</p>' !!}
      </div>
    </div>

    <!-- Edit Form -->
    <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl p-6 shadow space-y-6">
      @csrf
      @method('PUT')
      ...
      <div>
        <label for="judul" class="block text-sm font-medium text-gray-700 mb-2">Judul Service</label>
        <input type="text" id="judul" name="judul" value="{{ old('judul', $service->judul) }}" 
      ...
      <div>
        <label for="url" class="block text-sm font-medium text-gray-700 mb-2">Gambar Service</label>
        <input type="file" id="url" name="url" accept="image/*" 
               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F9B654] focus:border-transparent @error('url') border-red-500 @enderror">
        <p class="mt-1 text-sm text-gray-500">Pilih gambar untuk service ini (PNG/JPG)</p>
        @error('url')
          <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div>
        ...
        <input type="text" id="caption" name="caption" value="{{ old('caption', $service->caption) }}" 
