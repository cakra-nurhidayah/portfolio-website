{{-- resources/views/admin/skills_edit.blade.php --}}
...
<div class="bg-white/80 rounded-2xl shadow p-6">
  <div class="flex items-center justify-between mb-6">
    <h2 class="text-xl md:text-2xl font-bold text-[#262525]">Edit Skill</h2>
  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-xl p-6 shadow">
      <h3 class="text-2xl font-bold text-gray-800 mb-4">{{ $skill->judul }}</h3>
      
      @if($skill->url)
        <div class="relative group mb-4">
          <img src="{{ $skill->url }}" alt="{{ $skill->judul }}" class="w-full h-48 object-cover rounded-lg">
          @if($skill->caption)
            <div class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-75 text-white p-2 rounded-b-lg">
              <p class="text-sm">{{ $skill->caption }}</p>
            </div>
          @endif
        </div>
      @endif
    ...
    <div class="bg-white rounded-xl p-6 shadow">
      ...
      <div id="editor" contenteditable="true" class="min-h-96 p-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#F9B654] focus:border-transparent" 
        ...
        {!! old('deskripsi', $skill->deskripsi) ?: '<p>Mulai menulis deskripsi skill Anda di sini...</p>' !!}
      </div>
    </div>

    <!-- Edit Form -->
    <form method="POST" action="{{ route('admin.skills.update', $skill) }}" enctype="multipart/form-data" class="bg-white rounded-xl p-6 shadow space-y-6">
      @csrf
      @method('PUT')
      ...
      <div>
        ...
        <input type="text" id="judul" name="judul" value="{{ old('judul', $skill->judul) }}" 
      ...
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Pilih gambar untuk skill ini (PNG/JPG) - Biarkan kosong jika tidak ingin mengubah gambar</p>
        @error('url')
          <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div>
        ...
        <input type="text" id="caption" name="caption" value="{{ old('caption', $skill->caption) }}" 
